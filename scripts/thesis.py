...
class MicroVTune(ThesisExperimentSuite):
    ...
    def __init__(self, bag=None):
        super(self, MicroVTune).__init__([bag])

    def _get_kalman_filters(self):
        kalman_filters = []
        for micro_v in ThesisConfig.micro_v_list:
            kalman_filter = KalmanFilter(
                ThesisConfig.get_Q_k(99999, 99999), ThesisConfig.R_k,
    ...
    def _get_experiment_suite(self):
        experiments = []
        for kalman_filter in self._kalman_filters:
            experiment = Experiment(
                self._bags_sys_io,
                kalman_filter,
                ThesisConfig.micro_v_slice,
                ThesisConfig.micro_v_legend)
