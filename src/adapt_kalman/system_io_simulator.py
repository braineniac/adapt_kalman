...
class SystemIOSimulator(object):
    ...
    def get_stamped_input(self):
        return (self._time,self._input)

    def get_stamped_output(self):
        return (self._time,self._output)

    def _set_stamped_input(self):
        raise NotImplementedError

    def _set_stamped_output(self):
        raise NotImplementedError
...
class LineSimulator(SystemIOSimulator):
    ...
    def __init__(self,time=None, peak_vel=None):
        if peak_vel is None:
            ...
            super(LineSimulation,self).__init__(time)
            self._peak_vel = peak_vel

    def _set_stamped_input(self):
        box_function = get_boxcar(t,0.8,self._peak_vel)
